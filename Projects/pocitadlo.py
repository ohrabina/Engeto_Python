...
"""
oddelovac: str = "+--+----------+--+"
vycistene_slova = list()

for slovo in text.split():
    vycistene_slova.append(slovo.strip(".,:;").lower())

vyskyt_slov = dict()
for slovo in vycistene_slova:
    if slovo not in vyskyt_slov:
        vyskyt_slov[slovo] = 1
    elif slovo in vyskyt_slov:
        vyskyt_slov[slovo] = vyskyt_slov[slovo] + 1

devet_nejcastejsich = sorted(list(vyskyt_slov.values()), reverse=True)[:9]

vysledky = list()
for vyskyt in vyskyt_slov:
    if vyskyt_slov[vyskyt] in devet_nejcastejsich:
